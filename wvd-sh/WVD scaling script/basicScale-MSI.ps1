...
$Securepass1 = (Get-AzureKeyVaultSecret -VaultName $KeyVaultName -Name $Username).SecretValue
$Credential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList($Username, $Securepass1)
...
if (!$isServicePrincipalBool) {
  ...
  try {
    ...
    $Securepass = (Get-AzureKeyVaultSecret -VaultName $KeyVaultName -Name $Username).SecretValue
  ...
  try {
    ...
    $AppSecurepass = (Get-AzureKeyVaultSecret -VaultName $KeyVaultName -Name $AADApplicationID).SecretValue
    $appCreds = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList($AADApplicationID, $AppSecurepass)
    $azureauthentication = Connect-AzureRmAccount -ServicePrincipal -Credential $appCreds -TenantId $AADTenantId
